...
import { fetchServerStatus, fetchAllServersStatus, formatStatusText } from './status.js';
import {
  fetchServerErrors,
...
program
  ...
  .option('--api-key <key>', 'server API key (WARNING: visible in shell history; prefer prompt)')
...
program
  ...
  .option('--folder-id <id>', 'Syncthing folder ID (auto-generated from label if omitted)')
  ...
  .option('--server-key <key>', 'override server API key (not saved; visible in shell history!)')
  ...
  .action(async (opts) => {
    ...
    if (opts.serverKey) {
      ...
      console.error(`Unknown server: ${opts.server}. Use: stx add-server ... or --server-key`);
      process.exit(1);
    }
    ...
    if (opts.ssh) {
      ...
      const riskyPaths = ['/', '/root', '/home', '/var', '/etc', '/usr', '/tmp'];
      if (riskyPaths.includes(serverPath) || riskyPaths.some((p) => serverPath === p + '/')) {
...
program
  ...
  .action(async (opts) => {
    ...
    if (opts.server) {
      ...
      const status = await fetchServerStatus(opts.server, server.url, server.apiKey);
      if (opts.json) {
  ...
  .action(async (opts) => {
    ...
    if (opts.clear) {
      if (opts.server) {
        ...
        const result = await clearServerErrors(opts.server, server.url, server.apiKey);
        if (opts.json) {
          console.log(JSON.stringify(result, null, 2));
        } else {
          console.log(
            result.cleared ? `Cleared errors on ${opts.server}` : `Failed: ${result.error}`
          );
        }
      } else {
        ...
        if (opts.json) {
          ...
          for (const r of results) {
            console.log(
              r.cleared ? `Cleared errors on ${r.server}` : `${r.server}: Failed - ${r.error}`
            );
          }
        }
      }
      ...
      if (opts.server) {
        ...
        const errors = await fetchServerErrors(opts.server, server.url, server.apiKey);
        if (opts.json) {
...
program
  ...
  .action(async (opts) => {
    ...
    const events = await fetchServerEvents(opts.server, server.url, server.apiKey, {
      since: opts.since,
      limit: opts.limit,
      types,
    });
...
program
  ...
  .action((shell: string) => {
    ...
    if (!validShells.includes(shell)) {
      console.error(`Invalid shell: ${shell}. Use 'bash' or 'zsh'.`);
      process.exit(1);
    }
